...
class GNNModel(Model):
    def __init__(self, hidden_size, num_classes, num_edge_features):
    ...
    def call(self, x, edge_index, edge_features):
        ...
        for i in range(self.num_edge_features):
            one_hot_encoded_feature = tf.one_hot(edge_features[:, i], depth=NUM_BINS)
        ...
        encoded_edge_features_aggregated = tf.math.unsorted_segment_sum(encoded_edge_features, segment_ids_mapped, num_segments=tf.reduce_max(segment_ids_mapped) + 1)
...
def preprocess_data(num_agents, num_substations):
    ...
    return agent_data['chromosome'], edge_index, edge_features
...
def main():
    ...
    gnn_model = GNNModel(hidden_size, num_classes, num_edge_features=edge_features.shape[1])
